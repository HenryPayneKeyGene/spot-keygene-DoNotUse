...
class GraphNavInterface(object):
    ...
    def __init__(self, robot: Spot, upload_path):
        self.spot = robot
        ...
        power_state = self.spot.state_client.get_robot_state().power_state
    ...
    def _set_initial_localization_fiducial(self, *args):
        ...
        self.spot.graph_nav_client.set_localization(initial_guess_localization=localization,
    ...
    def _set_initial_localization_waypoint(self, *args):
        ...
        self.spot.graph_nav_client.set_localization(
    ...
    def _list_graph_waypoint_and_edge_ids(self, *args):
        ...
        localization_id = self.spot.graph_nav_client.get_localization_state().localization.waypoint_id
    ...
    def _upload_graph_and_snapshots(self, *args):
        ...
        response = self.spot.graph_nav_client.upload_graph(graph=self._current_graph,
                                                           generate_new_anchoring=true_if_empty)
        # Upload the snapshots to the robot.
        for snapshot_id in response.unknown_waypoint_snapshot_ids:
            waypoint_snapshot = self._current_waypoint_snapshots[snapshot_id]
            self.spot.graph_nav_client.upload_waypoint_snapshot(waypoint_snapshot)
            print(f'Uploaded {waypoint_snapshot.id}')
        for snapshot_id in response.unknown_edge_snapshot_ids:
            edge_snapshot = self._current_edge_snapshots[snapshot_id]
            self.spot.graph_nav_client.upload_edge_snapshot(edge_snapshot)
            print(f'Uploaded {edge_snapshot.id}')

        # The upload is complete! Check that the robot is localized to the graph,
        # and if it is not, prompt the user to localize the robot before attempting
        # any navigation commands.
        localization_state = self.spot.graph_nav_client.get_localization_state()
    ...
    def _navigate_to_anchor(self, *args):
        ...
        while not is_finished:
            ...
            try:
                nav_to_cmd_id = self.spot.graph_nav_client.navigate_to_anchor(
    ...
    def _navigate_to(self, *args):
        ...
        while not is_finished:
            ...
            # navigation command (with estop or killing the program).
            try:
                nav_to_cmd_id = self.spot.graph_nav_client.navigate_to(destination_waypoint, 1.0,
    ...
    def _navigate_route(self, *args):
        ...
        if all_edges_found:
            ...
            route = self.spot.graph_nav_client.build_route(waypoint_ids, edge_ids_list)
            is_finished = False
            while not is_finished:
                # Issue the route command about twice a second such that it is easy to terminate the
                # navigation command (with estop or killing the program).
                nav_route_command_id = self.spot.graph_nav_client.navigate_route(
    ...
    def _clear_graph(self, *args):
        ...
        return self.spot.graph_nav_client.clear_graph()
    ...
    def toggle_power(self, should_power_on):
        ...
        if not is_powered_on and should_power_on:
            ...
            power_on_motors(self.spot.power_client)
            motors_on = False
            while not motors_on:
                future = self.spot.state_client.get_robot_state_async()
            ...
            # powered-on state.
            safe_power_off_motors(self.spot.command_client, self.spot.state_client)
        else:
    ...
    def _check_success(self, command_id=-1):
        ...
        if command_id == -1:
            ...
            return False
        status = self.spot.graph_nav_client.navigation_feedback(command_id)
    ...
    def _on_quit(self):
        ...
        # Sit the robot down + power off after the navigation command is complete.
        if self._powered_on and not self._started_powered_on:
            self.spot.command_client.robot_command(RobotCommandBuilder.safe_power_off_command(),
